...
const CHANNELS = [
  { label: 'Email', value: 'info@embratechnologies.org', href: 'mailto:info@embratechnologies.org', icon: <><rect x="3" y="5" width="18" height="14" rx="2" /><path d="m4 7 8 6 8-6" /></> },
  ...
  { label: 'Website', value: 'embratechnologies.org', href: 'https://embratechnologies.org', icon: <><circle cx="12" cy="12" r="10" /><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" /></> },
];
...
export default function ContactClient() {
  ...
  const submit = async (e) => {
    ...
    setError('');
    try {
      const res = await fetch('https://formsubmit.co/ajax/sales@embratechnologies.org', {
        method: 'POST',
    ...
    <div ref={ref} className="page-shell">
      ...
      <section className="contact-section">
        <div className="wrap contact-grid">
          ...
          <form className="contact-form reveal-up" onSubmit={submit}>
            {sent ? (
              ...
              <>
                <div className="form-row">
                  <div className="form-field">
                    ...
                    <input id="cf-name" type="text" required placeholder="John Smith" value={form.name} onChange={update('name')} />
                  </div>
                  <div className="form-field">
                    <label htmlFor="cf-email">Email Address</label>
                    <input id="cf-email" type="email" required placeholder="john@business.com" value={form.email} onChange={update('email')} />
                  </div>
                </div>
                <div className="form-field">
                  <label htmlFor="cf-business">Business Name &amp; Website</label>
                  <input id="cf-business" type="text" placeholder="Smith Plumbing — smithplumbing.com" value={form.business} onChange={update('business')} />
                </div>
                <div className="form-field">
                  <label htmlFor="cf-message">What do you need?</label>
                  <textarea id="cf-message" rows="5" required placeholder="Tell us about your goals — a new website, better Google rankings, more leads..." value={form.message} onChange={update('message')} />
                </div>
                ...
                {error && <p style={{ color: '#FB7185', fontSize: 13.5, marginTop: 14 }}>{error}</p>}
              </>
            )}
          </form>
